...
def kiclist(network=None):
    ...
    example:
    --------
    In[1]: kics = kiclist(network=None)
    ----------------------------------------------------------------------
    """
...
def isthere(kicin,short=False,network=None,clip=False):
    ...
    example:
    --------
    In[1]: isthere(9821078,short=True,network='gps',clip=True)
    ----------------------------------------------------------------------
    """
   ...
   # Define file tags 
    ...
    if network == 'astro':
        path = '/scr2/jswift/EBs/outdata/'+str(kicin)+'/Refine/'
    if network == 'gps':
        path = '/home/jswift/EBs/outdata/'+str(kicin)+'/Refine/'
    if network == None:
        path = '/Users/jonswift/Astronomy/EBs/outdata/'+ \
        str(kicin)+'/Refine/'
    if network == 'kic':
        path = '/Users/jonswift/Astronomy/EBs/KIC'+str(kicin)+ \
        '/lc_fit/outdata/'+str(kicin)+'/Refine/'
...
def get_kic_info(kic,short=False,network=None,clip=False,
    ...
    example:
    --------
    In[1]: lc,pdata,sdata = get_kic_info(9821078,short=True,network=None,
...
# Setup path and info specific to KOI
    if network == 'astro':
        path = '/scr2/jswift/EBs/outdata/'+str(koi)+'/'
    if network == 'gps':
        path = '/home/jswift/EBs/outdata/'+str(koi)+'/'
    if network == None:        
        path = '/Users/jonswift/Astronomy/EBs/outdata/'+str(koi)+'/'
    if network == 'kic':
        path = '/Users/jonswift/Astronomy/EBs/KIC'+str(koi)+'/lc_fit/outdata/'+str(kic)+'/'
...
# Take ephemeris nearest to the midpoint of time series
# (this is a kludgey way to do this)
...
# Get fiducial limb darkening reference points
    print ""
    if lprior:
        print "Getting distribution of LD coefficients from stellar parameters for LD prior"
...
def compute_eclipse(info,unsmooth=False):
    ...
    parm[eb.PAR_J]      =  info["J"]     # J surface brightness ratio
    parm[eb.PAR_RASUM]  =  info["RASUM"] # (R_1+R_2)/a
    parm[eb.PAR_RR]     =  info["RR"]    # R_2/R_1
    parm[eb.PAR_COSI]   =  info["COSI"]  # cos i
    ...
    if limb == 'nlin':
        ...
        q1in = ldc[0]
        q2in = ldc[1]
        u1 =  2*np.sqrt(q1in)*q2in
        u2 =  np.sqrt(q1in)*(1-2*q2in)
    ...
    # Sample transit at "modelfac" times the integration time else if impact parameter is < 1-rl
    # Do not allow grazing transits
    if impact >= (1+rl):
        duration = 0.0
        sztrans = 10
    ...
    # Relative times of contact points and mid ingress/egress time
    xtot  = np.sqrt((1+rl)**2 - impact**2)
    if impact <= (1-rl):
        xfull = np.sqrt((1-rl)**2 - impact**2)
    else: 
        xfull = 0.0

#    xmid = (xtot+xfull)/2.

    # full transit time / mid egress time
#    dxfac = xtot/xmid
    ...
    # Time vector (assumption linear with btmp = constant a/R* approximation)
...
## Return tII to tIII duration if requested 
#    if getcontact:
#        func = sp.interpolate.interp1d(b0,b0time,kind='cubic')
#        if np.min(b0) >= (1. - rl):
#            tfull = 0.0
#        else:
#            ttot  = 2.0 * func(1.0 + rl)
#            tfull = 2.0 * func(1.0 - rl)
#        return tmodel+t0,smoothmodel,np.array([ttot,tfull])
#    else:
...
# Input parameters
    ...
    per      = x[4]
#    if LDdraw == True:
#        q1 = q1s[np.round(np.random.uniform(0,1,1)[0]*len(q1s))]
#        q2 = q2s[np.round(np.random.uniform(0,1,1)[0]*len(q2s))]
#        ldc = [q1,q2]
#    else:
    q1       = x[5]
    q2       = x[6]
...
### Compute transit model for given input parameters ###
    ...
    tmodel,smoothmodel,model = compute_trans(rprs,duration,impact,t0,per,ldc,unsmooth=True)
...
#        I = limbvals(angles,ldc,limbmodel=limb)
#        lfl = -1*(I - Imu0)**2/(2.0*(Isig*2)**2)
#        lf = lf + np.sum(lfl)
...
def plot_model(modelparams,short=False,tag='',markersize=5,smallmark=2,nbins=100,errorbars=False):
...
# Get model, raw (unsmoothed) model, and tfull
    ...
    tmodel,model,rawmodel = compute_trans(modelparams[0],modelparams[1],modelparams[2],\
                                          0.0,modelparams[4],\
                                          modelparams[5],unsmooth=True)

    tfull = tottofull(modelparams[1],modelparams[0],modelparams[2])
...
def get_limb_qs(Mstar=0.5,Rstar=0.5,Tstar=3800.0,limb='quad',network=None):
    ...
    if limb == 'nlin':
        a,b,c,d = glc.get_limb_coeff(Tstar,loggstar,network=network,limb=limb,interp='linear')
        return a,b,c,d
    else:
        a,b = glc.get_limb_coeff(Tstar,loggstar,network=network,limb=limb,interp='linear')
...
def bestvals(chains=False,lp=False,network=None,bindiv=10.,thin=False,
             frac=0.001,nbins=100,rpmax=1,durmax=10):
...
# Plot initial fit
    ...
    if limb == 'nlin':
        plot_model([pdata[0,0],tmid,pdata[0,2],0.0,pdata[0,4],\
                        [sdata[0,3],sdata[0,4],sdata[0,5],sdata[0,6]]],tag='_mpfit',nbins=nbins)
    else:
        q1 = min(max((sdata[0,3]+sdata[0,4])**2,0),1)
        q2 = min(max(sdata[0,4]/(2.0*(sdata[0,3]+sdata[0,4])),0),1)
        plot_model([pdata[0,0],tmid,pdata[0,2],0.0,pdata[0,4],[q1,q2]],tag='_mpfit',nbins=nbins)
...
#  Get maximum likelihood values
    ...
    nb = np.ceil((np.max(rprsdist)-np.min(rprsdist)) / (np.abs(rprshi-rprslo)/bindiv))
...
#    plt.axvline(x=rprshi,color='c',linestyle='--')
    ...
    nb = np.ceil((np.max(ddist)-np.min(ddist)) / (np.abs(dhi-dlo)/bindiv))
...
#    plt.axvline(x=dhi,color='c',linestyle='--')
   ...
   # Impact parameter                        
    ...
    nb = np.ceil((np.max(bdist)-np.min(bdist)) / (np.abs(bhi-blo)/bindiv))
...
#    plt.axvline(x=thi,color='c',linestyle='--')
 ...
 # Period
    ...
    nb = np.ceil((np.max(pdist)-np.min(pdist)) / (np.abs(phi-plo)/bindiv))
...
#    plt.axvline(x=phi,color='c',linestyle='--')
 ...
 # q1
    ...
    nb = np.ceil((np.max(q1dist)-np.min(q1dist)) / (np.abs(q1hi-q1lo)/bindiv))
...
# q2
    ...
    nb = np.ceil((np.max(q2dist)-np.min(q2dist)) / (np.abs(q2hi-q2lo)/bindiv))
...
#    plt.axvline(x=q1hi,color='c',linestyle='--')
   ...
   # calculate limb darkening parameters
    if limb == 'quad':
        u1 =  2*np.sqrt(q1val)*q2val
        u2 =  np.sqrt(q1val)*(1-2*q2val)
        ldc = [u1,u2]
    if limb == 'sqrt':
        u2 =  2*np.sqrt(q1val)*q2val
        u1 =  np.sqrt(q1val)*(1-2*q2val)
        ldc = [u1,u2]
    if limb == 'nlin':
 ...
 # Plot up final fit
    if limb == 'nlin':
        plot_model([vals[0],vals[1],vals[2],vals[3]-pdata[0,3],vals[4],\
                        [vals[5],vals[6],vals[7],vals[8]]],tag='_MCMCfit')
    else:
        plot_model([vals[0],vals[1],vals[2],vals[3]-pdata[0,3],vals[4],[vals[5],vals[6]]],tag='_MCMCfit')
...
def triangle_plot(chains=False,lp=False,thin=False,frac=0.001):
    ...
    t = time.time()
    print "... first column"
    column_plot(rprsdist,ddist,gs[1,0],val1=rprsval,val2=dval,ylabel=r'$\tau_{\rm tot}$ (h)',frac=frac)
    print done_in(t)
    ...
    row_plot(ddist,bdist,gs[6,1],val1=dval,val2=bval,xlabel=r'$\tau_{\rm tot}$ (h)',frac=frac)
    print done_in(t2)
    ...
    plt.savefig(path+'MCMC/'+name+stag+ctag+rtag+lptag+ltag+'fit_triangle.png', dpi=300)
    plt.savefig(path+'MCMC/'+name+stag+ctag+rtag+lptag+ltag+'fit_triangle.eps', format='eps', dpi=600)
...
def do_fit(koi,planet,nwalkers=1000,burnsteps=1000,mcmcsteps=1000,clobber=False,
           network=None,thin=10,frac=0.001,clip=False,limbmodel='quad',doplots=False,
           short=False,rprior=False,lprior=False,notriangle=True,
           errfac=3,rpmax=0.5,durmax=10,bindiv=10):
    ...
    if check1:
        ...
        chains,lp = fit_single(nwalkers=nwalkers,burnsteps=burnsteps,\
                                   mcmcsteps=mcmcsteps,clobber=clobber)
        if doplots != False:
            fit,rpdf = bestvals(chains=chains,thin=thin,frac=frac,rpmax=rpmax,
